...
- The hypothesis MUST explicitly reference at least THREE distinct entities, variables,
or mechanisms introduced in the prior hypotheses, and their roles must be causally necessary to the claim.
- The hypothesis MUST introduce a new variable or constraint, and explain its removal or effect relative to the Prior Hypotheses.
- The hypothesis MUST state the reason of these new constraints/variables from the Prior Hypothesis (very crucial).
- The hypothesis MUST assert a non-trivial dependency, tradeoff, or constraint that was not already explicit in the prior hypotheses.
- The hypothesis MUST stay in topic.
- DO NOT include an assumption that is not related to the Prior Hypothesis.
- Do NOT restate, summarize, justify, or explain.
...
DEFAULT_TREE = [
    [
        "Light",
        "Axiom 1 (Existence of Light): Light is a physical phenomenon that allows objects to be seen. It propagates through space and can travel through certain materials such as air, glass, or water. We assume light exists independently of observers and can be studied by measuring its effects, such as illumination, heating, or chemical change.",

        "Axiom 2 (Rectilinear Propagation in Uniform Media): In a uniform medium, light travels in straight lines unless it interacts with matter. This assumption explains the formation of sharp shadows and is valid when variations in material properties are negligible over the scale of interest.",

        "Axiom 3 (Interaction with Matter): When light encounters matter, it may be reflected, transmitted, absorbed, or scattered. The outcome depends on the properties of the material and the characteristics of the light itself. These interactions can change the direction, intensity, or color of the light.",

        "Empirical Observation: White light from the Sun or a lamp appears colorless to the eye, yet when passed through a glass prism, it spreads into a continuous range of colors. This suggests that white light is not fundamental but composed of multiple components with different properties.",

        "Newton’s Prism Experiment: By passing a narrow beam of sunlight through a triangular prism, Newton observed that different colors emerge at different angles and that recombining these colors restores white light. This establishes that white light is a superposition of many color components rather than being modified by the prism.",

        "Wave Model of Light (Foundational Assumption): Light can be modeled as an electromagnetic wave characterized by a wavelength λ and frequency ν. Different colors correspond to different wavelengths, with shorter wavelengths perceived as blue and longer wavelengths as red.",

        "Mathematical Representation: An incident beam of white light can be represented as a superposition of monochromatic plane waves with wavelengths λ ∈ [λ_min, λ_max]. Each component propagates independently in linear media, allowing analysis wavelength by wavelength.",

        "Law of Refraction (Snell’s Law): When light crosses an interface between two media, the angles of incidence θ_i and refraction θ_r satisfy n₁ sin(θ_i) = n₂ sin(θ_r), where n is the refractive index of the medium. This law is an empirical rule that connects geometry with material response.",

        "Dispersion: The refractive index n depends on wavelength, written as n = n(λ). Because dn/dλ ≠ 0 for real materials, different wavelengths refract at different angles. This wavelength dependence is the fundamental cause of spectral separation in prisms.",

        "Assumptions for Validity: The incident beam is collimated, the medium is isotropic and homogeneous, and magnetic effects are negligible (μ ≈ μ₀). Under these assumptions, geometric optics and wavelength-wise analysis are justified.",

        "Extension to Real Materials: In real media, light does not only change direction but may lose energy. This is modeled by introducing a complex refractive index ñ(λ) = n(λ) + iκ(λ), where κ(λ) quantifies attenuation due to absorption.",

        "Absorption Law: The absorption coefficient is defined as α(λ) = 4πκ(λ)/λ. As light propagates a distance x through a homogeneous absorbing medium, the intensity of each wavelength decays according to the Beer–Lambert law: I(λ, x) = I₀(λ) exp[−α(λ)x].",

        "Limitation of Pure Absorption Models: Beer–Lambert law assumes that photons are either transmitted or absorbed, but not redirected. In many real-world media (fog, milk, biological tissue), light is also scattered, violating this assumption.",

        "Introduction of Scattering: Scattering redirects light into new directions without necessarily removing its energy. To describe this, define absorption coefficient σ_a(λ), scattering coefficient σ_s(λ), and extinction coefficient σ_t(λ) = σ_a(λ) + σ_s(λ).",

        "Radiative Transfer Framework: The propagation of light in scattering and absorbing media is governed by the Radiative Transfer Equation (RTE), which describes how the spectral radiance L(λ, r, Ω) changes along a path due to extinction, in-scattering, and sources.",

        "Scattering Regimes: The wavelength dependence of σ_s(λ) depends on particle size. For particles much smaller than the wavelength, Rayleigh scattering applies (σ_s ∝ λ⁻⁴), while for particles comparable in size to λ, Mie theory is required.",

        "Observable Consequence: Because shorter wavelengths scatter more strongly, scattering-dominated media preferentially remove blue light from the forward direction, altering the observed spectrum even when absorption is weak.",

        "Measurement Model: The transmitted spectrum after passing through a medium of thickness d is given by I_out(λ) = I_in(λ) · exp[−σ_t(λ)d] · T_sys(λ), where T_sys accounts for detector and optical system response under the assumption of linearity.",

        "Derived Observable: Define the spectral centroid λ_c = (∫λ I(λ)dλ) / (∫I(λ)dλ). A shift in λ_c between input and output spectra quantifies color change but does not uniquely identify whether absorption or scattering dominates.",

        "Experimental Strategy: By independently varying optical path length d and particle concentration, and by measuring angular and polarization-resolved spectra, one can separate absorption-dominated effects from scattering-dominated ones.",

        "Inference Challenge: Estimating σ_a(λ) and σ_s(λ) from measured data requires solving an inverse problem based on the RTE, which is ill-posed and requires regularization and physically motivated constraints.",

        "Given these axioms, laws, and observational limits, what is the next governing equation or approximation that can be introduced to simplify the Radiative Transfer Equation for optically thin media and explicitly relate measured angular intensity to σ_s(λ)?"
    ],
...
def expand_existing_raw_tree(llm: LLMInterface,
    ...
    for step in range(max_depth):
        ...
        for attempt in range(MAIN_ATTEMPTS):
            cand_raw = llm.generate(prompt, max_new_tokens=512, temperature=0.7)
        ...
        if branch_factor and branch_factor > 1:
            ...
            for bi in range(branch_factor - 1):
                ...
                for attempt in range(SIB_ATTEMPTS):
                    sib_raw = llm.generate(prompt, max_new_tokens=256, temperature=0.9)
...
def parse_args():
    ...
    p.add_argument("--lora_path", type=str, default="out_checkpoints/dpo_epoch1")
    p.add_argument("--lora_only", action="store_true", default=False)
    return p.parse_args()
